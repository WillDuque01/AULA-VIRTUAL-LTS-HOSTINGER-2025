<x-app-layout>
    <div class="py-12">
        <div class="mx-auto max-w-2xl space-y-6 rounded-3xl border border-emerald-100 bg-white p-8 text-center shadow-sm">
            ...
            <h1 class="text-2xl font-semibold text-slate-900">{{ __('shop.checkout.title') }}</h1>
            @php
                $summary = session('checkout_success');
            @endphp
            <p class="text-sm text-slate-500">
                {{ __('shop.checkout.description') }}
            </p>
            @if($summary)
                <p class="text-sm text-slate-600">
                    {{ trans_choice(__('shop.checkout.summary'), $summary['count'] ?? 1, ['count' => $summary['count'] ?? 1]) }} ·
                    ${{ number_format($summary['total'] ?? 0, 2) }} USD ·
                    {{ __('shop.checkout.method', ['method' => $summary['method'] === 'transfer' ? __('shop.checkout.method_transfer') : __('shop.checkout.method_card')]) }}
                </p>
            @endif
