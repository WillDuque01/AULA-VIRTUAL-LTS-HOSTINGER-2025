...
@section('content')
    ...
    @component('emails.components.panel')
        <p style="margin: 0 0 6px 0; font-size: 15px; color: #38bdf8;">
            {{ __('email.blocks.payment.tier', ['tier' => $tierName ?? __('email.blocks.payment.plan_default')]) }}
        </p>
        @if(!empty($amount) && !empty($currency))
            <p style="margin: 0 0 6px 0; font-size: 14px; color: #cbd5f5;">
                {{ __('email.blocks.payment.amount', ['amount' => $amount, 'currency' => $currency]) }}
            </p>
        @endif
        <p style="margin: 0 0 6px 0; font-size: 14px; color: #94a3b8;">
