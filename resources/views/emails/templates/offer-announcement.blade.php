...
@section('content')
    ...
    @component('emails.components.panel')
        <h3 style="margin: 0 0 12px 0; color: {{ $emailPalette['accent'] }}; font-size: 16px;">{{ $offerTitle }}</h3>
        <p style="margin: 0 0 12px 0; font-size: 14px; line-height: 1.6; color: {{ $emailPalette['text'] }};">{!! $offerDescription !!}</p>
        @if($validUntil)
            <p style="margin: 0 0 6px 0; font-size: 13px; line-height: 1.6; color: {{ $emailPalette['muted'] }};">{{ __('email.blocks.offer.valid_until', ['date' => $validUntil]) }}</p>
        @endif
        @if($price)
            <p style="margin: 0 0 6px 0; font-size: 14px; line-height: 1.6; color: {{ $emailPalette['accent'] }}; font-weight: 600;">{{ __('email.blocks.offer.price', ['amount' => $price]) }}</p>
        @endif
        @if($discount)
            <p style="margin: 0; font-size: 13px; line-height: 1.6; color: {{ $emailPalette['success'] }}">{{ __('email.blocks.offer.discount', ['value' => $discount]) }}</p>
        @endif
    @endcomponent
