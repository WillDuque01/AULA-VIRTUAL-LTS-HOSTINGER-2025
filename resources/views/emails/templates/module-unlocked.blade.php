...
@section('content')
    ...
    @component('emails.components.panel')
        <h3 style="margin: 0 0 12px 0; color: {{ $emailPalette['accent'] }}; font-size: 16px;">{{ $moduleTitle }}</h3>
        <p style="margin: 0 0 6px 0; font-size: 13px; line-height: 1.6; color: {{ $emailPalette['text'] }}">{{ $courseTitle }}</p>
