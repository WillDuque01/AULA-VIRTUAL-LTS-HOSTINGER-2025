...
- **Próximo hito**: investigar y diseñar un instalador web tipo “one-click” (similar a WordPress) que permita desplegar nuevas academias en planes Cloud Startup de Hostinger a partir de un paquete ZIP y un script `installer.php`. La viabilidad depende de las capacidades/restricciones del hosting (ejecución de `shell_exec`, límites de tiempo, permisos sobre `composer`, etc.), por lo que se planificó un estudio riguroso del blueprint oficial de Hostinger.
- **Decisión arquitectónica**: el instalador será un módulo autónomo (wizard) que no altera el core del LMS. El despliegue manual/CI seguirá disponible; el wizard simplemente ofrece una tercera vía “one-click”. Tras instalar, el script se autodestruye y deja el LMS intacto. Prototipos CLI y web vivirán en carpetas independientes para no contaminar el código base.

Con esta memoria, cualquier integrante puede retomar el proyecto entendiendo arquitectura, flujos críticos, documentación disponible y focos de evolución inmediata, además del estado de monitoreo post-entrega. El prototipo CLI ya se completó y avanzamos al wizard web que coexiste con los métodos tradicionales de despliegue.
